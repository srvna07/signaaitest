...
export function TestCaseForm({ initialData, onSubmit, onCancel, submitLabel }: TestCaseFormProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [requirements, setRequirements] = useState<Requirement[]>([]);

  const [title, setTitle] = useState(initialData?.title || '');
  const [type, setType] = useState<'UI' | 'API'>(initialData?.type || 'UI');
  const [requirementId, setRequirementId] = useState(initialData?.requirementId || '');
  ...
  const [steps, setSteps] = useState<Omit<TestCaseStep, 'order'>[]>(
    initialData?.steps?.length ? initialData.steps : [{ action: '', expected: '' }],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await onSubmit({ title, type, requirementId, preconditions, expectedResult, steps });
    } catch (err: unknown) {
  ...
  return (
    ...
    >
      {error && (
        <div style={{ padding: '1rem', color: 'red', backgroundColor: '#fee2e2' }}>{error}</div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        <label style={{ fontSize: '0.85rem', fontWeight: 500 }}>Overall Expected Result *</label>
        <textarea
          required
          rows={3}
